...
class Trainer(object):
    ...
    def training(self, epoch):
        ...
        for i, (input, heatmap, centermap, img_path, _) in enumerate(tbar):
    ...
    def test_img(self, start_ts):
        self.model.eval()
        print("Testing")
        test_img_dir = "/DATA/test/ADK2021_TestDataSet/images"
        test_save_path = './datasets/cow'
        
        test_img_list = sorted(os.listdir(test_img_dir))
        annotation_li = []
        
        for ind, test_img_path in enumerate(test_img_list):
            print(ind,"/",1000)
            center   = [184, 184]
            
            img_orig = cv2.imread(os.path.join(test_img_dir, test_img_path))
            height, width, _ = img_orig.shape

            img  = np.array(cv2.resize(img_orig, (368,368)), dtype=np.float32)
            ...
            pred_kpts = get_kpts(heat, height, width)
            
            img_fn = test_img_path
            print(img_fn)
            annot = {
                "ID": ind + 1,
                "img_path": img_fn,
...
def main():        
    ...
    parser.add_argument('--train_dir', default='./datasets',type=str, dest='train_dir')
    parser.add_argument('--val_dir', type=str, dest='val_dir', default='./datasets')
    parser.add_argument('--test_dir', type=str, dest='test_dir', default='./datasets')
    ...
    if args.is_train == 1:
        ...
        print("*********************************\n")
        for epoch in range(starter_epoch, args.epochs):
        ...
        print("\n*********************************")
        print("===> Start test images for submission!")
        print("*********************************\n")
        trainer.test_img(start_ts=start_ts)
