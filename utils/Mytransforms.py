...
class RandomColorTransfer(object):
    def __init__(self, prob=0.5):
        this_dir = os.path.dirname(os.path.realpath(__file__))
        #print(this_dir)
        target = os.path.join(this_dir, 'cow.jpeg')
        target_img = cv2.imread(target)
        #print(target_img)
        self.prob = prob
        (self.lMeanTar, self.lStdTar, self.aMeanTar, self.aStdTar, self.bMeanTar, self.bStdTar) = getStdMean(target_img)
        self.color_transformer = color_transfer_constructor(self.lMeanTar, self.lStdTar, self.aMeanTar, self.aStdTar, self.bMeanTar, self.bStdTar, \
															l_range = (0.9, 1.1), a_range = (0.95, 1.05), b_range = (0.95, 1.05), constant_range = (-0.5, 0.5))
    
    def __call__(self, img, kpt, center):
        if random.random() < self.prob:
            return self.color_transformer(img), kpt, center
        return img, kpt, center
